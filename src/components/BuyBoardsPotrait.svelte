<script lang="ts">
	...
	const getTargetY = (index: number, length: number) => {
		...
		return (
			STACK_BASE_Y -
			(effectiveLength - 1) * ITEM_SPACING +
			index * ITEM_SPACING
		);
	};
	...
	const pulseToggle = () => {
		...
		void togglePulse.set(0.94, {
			duration: 60,
			easing: cubicOut,
		});

		void toggleAlpha.set(0.86, {
			duration: 60,
			easing: cubicOut,
		});
		...
		setTimeout(() => {
			...
			void togglePulse.set(1.18, {
				duration: 180,
				easing: backOut,
			});

			void toggleAlpha.set(1, {
				duration: 180,
				easing: cubicOut,
			});
		}, 60);
		...
		explosionStack.forEach((item, index) => {
			...
			if (newItemId !== undefined && item.id === newItemId) {
				setTimeout(() => {
					...
					void item.y.set(targetY, {
						duration: dropDuration,
						easing: cubicOut,
					});

					void item.scale.set(1, {
						duration: dropDuration,
						easing: cubicOut,
					});

					void item.alpha.set(1, {
						duration: 180,
						easing: cubicOut,
					});
				}, dropDelayMs);
	...
	const clearStackAnimated = () => {
		...
		requestAnimationFrame(() => {
			...
			explosionStack.forEach((item) => {
				void item.y.set(item.y.current + fallDistance, {
					duration,
					easing: cubicOut,
				});

				void item.alpha.set(0, {
					duration: duration - 120,
					easing: cubicOut,
				});

				void item.scale.set(0.96, {
					duration,
					easing: cubicOut,
				});
			});
	...
	context.eventEmitter.subscribeOnMount({
		symbolExplode: ({ data }) => {
			if (isClearingStack) {
				pendingExplodes = [
					{ symbol: data.symbol, value: data.value },
					...pendingExplodes,
				];
...
>
	<!-- ===== BUY BUTTONS ===== -->
	<Container x={12} scale={{ x: 0.9, y: 1.1 }}>
		<Container
			x={36}
			y={5}
			interactive={true}
	...
	>
		...
		<Rectangle
			...
			x={-65}
			y={-58}
			width={layout.width * 0.16}
			height={layout.height * 0.26}
		/>
		...
		<Container y={0}>
			{#each explosionStack as item (item.id)}
				<Container
					x={-15}
					y={item.y.current - 17}
					scale={item.scale.current}
					alpha={item.alpha.current}
				>
					<Sprite
						key="buyBoardPlaceHolder"
						anchor={0.5}
						scale={{ x: 0.285, y: 0.3 }}
					/>
